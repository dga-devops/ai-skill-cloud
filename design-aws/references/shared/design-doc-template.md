...
{{APP_NAME}} uses the `{{PATTERN}}` architecture pattern deployed across {{ENV_COUNT}} environments.

{{#PATTERN_ECS}}
{{SERVICE_COUNT}} service(s) on ECS Fargate behind ALB with path-based routing. CloudFront serves as CDN with per-service cache behaviors{{#WAF_ENABLED}} and WAF for security (pre-prod/prod){{/WAF_ENABLED}}.
{{/PATTERN_ECS}}
...
4. Generate stacks based on pattern resources
